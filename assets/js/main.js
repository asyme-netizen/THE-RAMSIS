...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderAccountPills(user = null) {
    accountPills.forEach((pill) => {
      ...
      pill.innerHTML = `
        <a class="account-profile-link" href="${profileHref}" aria-label="Open your profile">
          ${avatar
            ? `<img class="account-avatar-image" src="${avatar}" alt="${userDisplayName(user)}">`
            : `<span class="account-avatar-fallback">${initials}</span>`}
        </a>
        <a class="account-name-link" href="${profileHref}">
          <span class="account-label">Profile</span>
          <strong data-rendered-account-name>${userDisplayName(user)}</strong>
        </a>
