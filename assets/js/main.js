...
import { firebaseConfig, marketDataConfig } from "./firebase-config.js";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  async function loadBusinessMarkets() {
    ...
    if (tape) {
      tape.innerHTML = merged.map((item) => {
    ...
    if (grid) {
      grid.innerHTML = merged.slice(0, 6).map((item) => {
        const cls = marketClass(Number(item.changesPercentage || item.change || 0));
        const pct = Number(item.changesPercentage || 0);
        return `
  ...
  function renderTradingViewWidgets() {
    ...
    widgets.forEach((node) => {
      ...
      container.className = mode === "overview" ? "tradingview-widget-container" : "tradingview-widget-container";
